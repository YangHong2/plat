...
import com.dhlk.subcontract.sdk.*;
...
public class UnionpayServiceImpl implements UnionpayService {
    ...
    @Override
    public String payFor(String txnAmt) throws IOException {
        //前台页面传过来的
        String merId = "777290058110048";
        String orderId = this.nowTime();
        String txnTime = this.nowTime();

        Map<String, String> requestData = new HashMap<String, String>();
        /***银联全渠道系统，产品参数，除了encoding自行选择外其他不需修改***/
        requestData.put("version", AcpBase.version);   			  //版本号，全渠道默认值
        requestData.put("encoding", AcpBase.encoding); 			  //字符集编码，可以使用UTF-8,GBK两种方式
        ...
        requestData.put("frontUrl", AcpBase.frontUrl);
        ...
        requestData.put("backUrl", AcpBase.backUrl);
        ...
        Map<String, String> submitFromData = AcpService.sign(requestData, AcpBase.encoding);  //报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。
        String requestFrontUrl = SDKConfig.getConfig().getFrontRequestUrl();  //获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acpsdk.frontTransUrl
        String html = AcpService.createAutoFormHtml(requestFrontUrl, submitFromData, AcpBase.encoding);   //生成自动跳转的Html表单
    ...
    public Result payBack(HttpServletRequest req) throws IOException {
        ...
        if (!AcpService.validate(reqParam, encoding)) {
            ...
            //验签失败，需解决验签问题
            return ResultUtils.error("验证签名结果[失败]");
        } else {
            ...
            return ResultUtils.success("验证签名结果[成功]");
        }
    }

    @Override
    public Result payFront(Map<String, String> respParam) throws Exception {
        ...
        return ResultUtils.success(valideData);
    }
