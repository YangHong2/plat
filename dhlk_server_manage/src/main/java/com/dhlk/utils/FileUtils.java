...
public class FileUtils {
    ...
    @Value("${upoload.path.windows}")
    private String windowsPath;
    @Value("${upoload.path.linux}")
     ...
     * 读取sql文件是否包含某个关键字
     *
     * @param filePath sql的文件路径
     * @param items    关键字
     * @return
     */
    public static boolean isContainsSqlKey(String filePath, List<String> items) {
        boolean result = true;
        ...
        try {
            ...
            in.read(buffer);
            in.close();
            String str = new String(buffer, "UTF-8").toLowerCase();
     ...
     * @return
     */
    public static String fileParseStr(String filePath) {
        File file = new File(filePath);
        try {
            FileInputStream in = new FileInputStream(file);
            //size 为字串的长度,这里一次性读完
